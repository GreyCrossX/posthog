...
class ReplayFiltersEventsSubQuery(SessionRecordingsListingBaseQuery):
    ...
    def _get_person_id_based_sessions_query(
        ...
        """
        Two-stage query for person properties in PoE mode with hybrid approach:
        1. Find person_ids matching the properties (from recent events with those properties)
        2. Find all sessions for those person_ids (including sessions before identification)
