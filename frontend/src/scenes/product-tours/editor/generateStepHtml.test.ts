...
const mockHighlight = jest.fn(() => ({ type: 'root', children: [{ type: 'text', value: 'highlighted-code' }] }))
const mockHighlightAuto = jest.fn(() => ({ type: 'root', children: [{ type: 'text', value: 'auto-highlighted' }] }))
            ...
            content: [
                {
                    type: 'codeBlock',
                    attrs: { language: 'javascript' },
                    content: [{ type: 'text', text: '&lt;div&gt;&amp;test' }],
                },
            ],
        }

        generateStepHtml(content)

        expect(mockHighlight).toHaveBeenCalledWith('javascript', '<div>&test')
    })
})
